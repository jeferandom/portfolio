...
```mermaid
flowchart LR
    dev["Desarrollador"] -->|"push / PR"| gh["GitHub<br/>clicc-25 · clicc-backend · clicc-mailer"]
    gh -->|"webhook"| jenkins["Jenkins<br/>CI/CD"]
    jenkins -->|"docker build"| img["Imágenes Docker"]
    img -->|"push"| acr[("Azure Container Registry")]
    jenkins -->|"SSH + docker compose pull/up"| vm["Azure VM<br/>Ubuntu Server LTS"]
    acr -.->|"pull"| vm
```
...
```mermaid
flowchart TB
    user["Usuario"] -->|"HTTPS 443"| nginx["Nginx<br/>reverse proxy + SSL"]

    subgraph vm["Azure VM · Ubuntu Server LTS"]
        subgraph compose["Docker Compose"]
            nginx
            fe["Next.js 15 · PM2<br/>"]
            be["NestJS · PM2<br/> /api"]
            mailer["clicc-mailer · PM2<br/>Express + workers"]
            mongo[("MongoDB")]
        end
    end
    ...
    nginx -->|"/api"| be
    be --> mongo
    be -->|"POST /api/send-mail"| mailer
    mailer -->|"mailQueue"| mongo
    mailer -->|"SMTP"| o365["Office 365"]
```
